...
const launchBrowserIfNeeded = async function (debuglog) {
  ...
  if (!_browserLaunchPromise) {
    ...
    _browserLaunchPromise = _puppeteer2.default.launch(launchOptions).then(browser => {
...
const astFromCss = async function astFromCss(options, { debuglog, stdErr }) {
  ...
  try {
    ...
    normalizedCss = await (0, _normalizeCss2.default)({
      css,
      browser,
      debuglog
    });
...
const generateCriticalCssWrapped = async function generateCriticalCssWrapped(options, ast, { debuglog, stdErr, START_TIME, forceTryRestartBrowser }) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      formattedCss = await (0, _core2.default)({
        browser,
        url: options.url,
        astRules,
        width,
        ...
        timeout: timeoutWait,
        blockJSRequests: typeof options.blockJSRequests === 'undefined' ? DEFAULT_BLOCK_JS_REQUESTS : options.blockJSRequests,
...
const m = module.exports = function (options, callback) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const criticalCss = await generateCriticalCssWrapped(options, ast, logging);
